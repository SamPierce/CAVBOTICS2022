  ...
  public void execute() {
    ...
    if (RobotContainer.controller.getLeftStickButton()){
      ...
      if (light.hasTarget() == 1){
        ...
        double gyroStrafe = -1 * ((remote.getRightY()) * Math.sin(Math.toRadians(SwerveDrive.gyro.getAngle())) - (-1 * remote.getRightX()) * Math.cos(Math.toRadians(SwerveDrive.gyro.getAngle())));
